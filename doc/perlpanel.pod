...
=for comment
$Id: perlpanel.pod,v 1.17 2004/01/17 13:47:52 jodrell Exp $
...
=item * The Gnome2::Wnck Perl module for the tasklist, pager and
window menu applets

=item * Xmms::Remote for the XMMS applet
...
By default, PerlPanel will install into the C</usr/local> directory.
If you want to change this, you can either edit the C<PREFIX> variable
in the Makefile, or add 'C<PREFIX=/path/to/directory>' as an argument
to C<make>.

=head2 Installing from an RPM
...
the C<PerlPanel-x.y.x.src.rpm> file and run these commands:
...
missing Perl modules, even if they are already installed. To correct this,
use the C<--nodeps> argument to RPM.

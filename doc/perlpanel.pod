...
=for comment
$Id: perlpanel.pod,v 1.1 2003/06/13 15:43:33 jodrell Exp $
...
By default, PerlPanel will install into the /usr directory. If you want
to change this, you can either edit the PREFIX variable in the Makefile,
or add "PREFIX=/path/to/directory" as an argument to the "make" calls
above.

=head1 Running PerlPanel

Running PerlPanel is as simple as typing "perlpanel" into your terminal.
However you might wish to add this to your .xinitrc file or any other
scripts that start when your X session begins. =cut

=head1 Configuring PerlPanel

All configuration is done in the resource file, located in
$HOME/.perlpanelrc. This is an XML file and so it should be fairly easy
...
clicked, these launcher run the program for that icon.

IconBar uses the contents of $HOME/.perlpanel/icons to generate these
...
relative to $PREFIX/share/pixmaps.
...
"Edit" options require that the gnome-desktop-item-edit program be
available. It is expected that PerlPanel will have its own version of
...
panel itself. The Screen tab contains controls for the display width and
height. At the moment PerlPanel is unable to work these out for itself.
This is because the required functionality is not present in the
Gtk2-Perl libraries. When it becomes available this tab will disappear.

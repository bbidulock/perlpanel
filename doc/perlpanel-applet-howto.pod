...
=for comment
$Id: perlpanel-applet-howto.pod,v 1.17 2004/05/28 10:46:27 jodrell Exp $
...
PerlPanel doesn't yet (February 2004) have support for foreign
languages, but support is expected to be implemented soon. To prepare
for this support, PerlPanel provides a special function for retrieving a
...
Currently, all this function does is return the original string, with any
parameters replaced by their values.

When IL8N is properly implemented in PerlPanel it will use either gettext or
Locale::TextDomain, which is gettext-compliant. In either case, the usage of
this function is likely to change.

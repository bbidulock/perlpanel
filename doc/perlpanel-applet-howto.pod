...
=for comment
$Id: perlpanel-applet-howto.pod,v 1.9 2004/01/11 23:07:45 jodrell Exp $
...
C<$PREFIX/lib/perlpanel/applet.registry>. This file contains a short
...
C<$ok_callback> is executed if the user hits the 'Ok' button.
	...
	$PerlPanel::OBJECT_REF->get_widget_position($widget)
...
screen.
	...
	$PerlPanel::OBJECT_REF->get_mouse_pointer

This returns two numbers corresponding to the position on-screen of the
mouse pointer.

	$PerlPanel::OBJECT_REF->exec_wait($cmd, $callback)
...
is opened as a filehandle to be read from - so calling applications
which detach from STDOUT are not recommended. C<exec_wait> creates a Glib
handler and waits for C<eof> from the handle. Then it executes the function
in C<$callback>.

This function exists mainly because of C<IconBar.pm>.
=cut
